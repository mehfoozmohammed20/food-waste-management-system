    ...
    SECTIONS = {
        ...
        "Multivariate Analysis": [
            ("M1_city_provider_type_quantity", "City + Provider Type + Quantity", 90,
             "Among the top 8 cities by food quantity, only one city has multiple provider types active at the same time — the rest each depend on a single provider type for their entire food supply. This is a real vulnerability: in most of these high-supply cities, if that one dominant provider type were to pull back, the city's food supply would collapse entirely rather than just shrink, since there is no second provider type to fall back on."),
            ("M2_food_type_meal_type_quantity","Food Type + Meal Type + Quantity", 80,
             "Specific combinations stand out — both Non-Vegetarian and Vegan food peak at Snacks, while Vegetarian food peaks at Dinner instead. This means Snack-time listings are dominated by Non-Vegetarian and Vegan options, so Vegetarian receivers may find better selection by checking Dinner-time listings."),
            ("M3_provider_claims_quantity",    "Provider Type + Claim Status", 80,
             "Across all provider types, the ratio of completed versus cancelled claims stays fairly consistent, each within a few percentage points of the others. This tells us the high cancellation problem is not isolated to any specific provider category, it's a system-wide logistics or communication issue that needs a platform-level fix, not a provider-specific one."),
            ("M4_receiver_claims_quantity",    "Receiver Type + Claim Status", 80,
             "Charities have the highest claim completion rate at 35.8%, followed by Shelters at 34.3%, NGOs at 33.8%, and Individuals lowest at 31.3%. The gap between the best and worst receiver type is only about 4.5 percentage points, so while Charities are marginally more reliable, no single receiver type stands out dramatically from the rest."),
        ],
        "Claim Analysis": [
